...
char*token;
float currentLong, currentLat , speed, finalLat=3003.97187 ,finalLong=3116.74792;
...
while(1){
		...
		GPS_format();
	for(i=0;i<20;i++){
		UART0_write(GPS_formated[4][i]);}
		
		 long_arr[counter] = currentLong;
		 lat_arr[counter] = currentLat;
		//float  past long = current long;
		//float  past lat = current lat;
		if(counter!=0)
			{
				dist += GPS_getDistance(long_arr[counter-1],lat_arr[counter-1],long_arr[counter],lat_arr[counter]);
...
//				LCD_displayfloat(dist);/*display distance on lcd*/
			...
			//delay 3 sec for gps to recieve new data
